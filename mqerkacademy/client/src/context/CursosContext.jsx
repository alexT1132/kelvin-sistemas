...
const serializeTags = (val) => {
  if (Array.isArray(val)) return val.join(","); // CSV para columna TEXT
  return parseTags(val).join(",");
};

// Asegura que el shape que usa el frontend sea consistente
...
// Construye FormData solo cuando haya archivo (multer)
function buildFormData(payload) {
  ...
  if (payload.imagen instanceof File) {
    fd.append("imagen", payload.imagen); // field esperado por multer
...
export function CursosProvider({ children, initial = [] }) {
  ...
  const ObtenerCursos = useCallback(async () => {
    try {
      ...
      // Puede venir { ok, data: [...] } o directamente [...]
      ...
      // ⚠️ NO volver a llamarse a sí misma aquí
    } catch (err) {
  ...
  const EliminarCurso = useCallback(async (id) => {
    await deleteCursoRequest(id); // el backend ya borra imagen si existe
