...
export async function findCursoById(id) {
  const [rows] = await pool.execute("SELECT * FROM cursos WHERE id = ?", [id]);
  return rows[0] ? rowToEntity(rows[0]) : null;
}

export async function insertCurso(data) {
  const {
    nombre, codigo = "", subtitulo = "", modalidad = "PRESENCIAL",
    imagenUrl = "", tagsCSV = "", alumnos = 0, likes = 0, vistas = 0,
  ...
  const [result] = await pool.execute(
    `INSERT INTO cursos
     ...
     VALUES
      (?,      ?,      ?,         ?,         ?,         ?,    ?,       ?,     ?,
       ?,       ?,     ?,        ?,           ?)`,
    [nombre, codigo, subtitulo, modalidad, imagenUrl, tagsCSV, alumnos, likes, vistas,
     section, nivel, Number(duration), durationUnit, Number(rating)]
  );
...
export async function updateCursoById(id, data) {
  ...
  values.push(id);
  await pool.execute(`UPDATE cursos SET ${fields.join(", ")} WHERE id = ?`, values);
  return await findCursoById(id);
}

export async function deleteCursoById(id) {
  const [res] = await pool.execute("DELETE FROM cursos WHERE id = ?", [id]);
